...
func TestCatalogModelFromRequestRejectsOversizedBody(t *testing.T) {
	request := httptest.NewRequest(
		"POST",
		"/v1-catalog/catalogs",
		strings.NewReader(strings.Repeat("x", maxCatalogRequestBytes+1)),
	)

	if _, err := catalogModelFromRequest(request, "global"); err == nil {
		t.Fatal("oversized catalog request was accepted")
	}
}
